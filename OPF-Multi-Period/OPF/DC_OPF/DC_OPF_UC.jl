...
function DC_OPF_UC(
    ...
    @variable(m, y_sg[1:nG, 1:hours], Bin)   # start generating
    @variable(m, y_sd[1:nG, 1:hours], Bin)   # shutdown
    @variable(m, y_st[1:nG, 1:hours], Bin)   # startup (order)
    ...
    # --------------------------------------------------
    @constraint(m, [g in 1:nG, t in 1:hours], P_g[g,t] >= Pmin_pu[g] * y_g[g,t])
    @constraint(m, [g in 1:nG, t in 1:hours], P_g[g,t] <= Pmax_pu[g] * y_g[g,t])
    ...
    @constraint(m, [g in 1:nG, t in 2:hours],
        -(Pmsg_pu[g]*y_sd[g,t] + RR_pu[g]*y_g[g,t-1]) <= P_g[g,t] - P_g[g,t-1])

    @constraint(m, [g in 1:nG, t in 2:hours],
        P_g[g,t] - P_g[g,t-1] <= RR_pu[g]*y_g[g,t] + Pmsg_pu[g]*y_sg[g,t])
    ...
    for g in 1:nG
        if Tst[g] > 0
            for t in 1:hours
                if t > Tst[g]
                    @constraint(m, y_sg[g,t] == y_st[g, t - Tst[g]])
                end
            end
        end
    end
    ...
    for k in 1:nL
        if dLine.status[k] != 0
            f  = dLine.fbus[k]
            to = dLine.tbus[k]
            @constraint(m, [t in 1:hours], angmin[k] <= θ[f,t] - θ[to,t] <= angmax[k])
            rateA = dLine.rateA[k] / bMVA
            Bij   = B[f,to]
            @constraint(m, [t in 1:hours], -rateA <= Bij*(θ[f,t] - θ[to,t]) <= rateA)
        end
    end
    ...
    @objective(m, Min,
        ...
        sum(Cstart[g]*y_sg[g,t]   for g in 1:nG, t in 2:hours))
    ...
    if termination_status(m) in (MOI.OPTIMAL, MOI.LOCALLY_SOLVED, MOI.ITERATION_LIMIT)
        for t in 1:hours
            ...
            cost_t = 0.0
            for g in 1:nG
                cost_t += Cm[g]*value(P_g[g,t])*bMVA + CnL[g]*value(y_g[g,t])
                if t >= 2
                    cost_t += Cstart[g]*value(y_sg[g,t])
                end
            end
